...
BEGIN
  ...
  ${@.variable.01-privs.name}.expand(1);
  ${@.variable.01-privs.name}( ${@.variable.01-privs.name}.last ) := '${i.value}';
  <%@ end-foreach %>

  ${@.variable.02-parent.name} := new ${@.variable.02-parent.data-type}();
  <%@ foreach( under, i ) %>
  ${@.variable.02-parent.name}.expand(1);
  ...
  obj.execution_snippet      := '$.procedure.secclass-create.exec';
